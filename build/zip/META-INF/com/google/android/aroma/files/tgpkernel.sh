...
if [ $OPTION == "check_n960f" ]; then
	echo "install=1" > $AROMA/n960f.prop
	exit 10
fi
...
if [ $OPTION == "rom_check" ]; then
	...
	# Set for N9 ROMs
	if grep -q ro.build.product=crown $BUILDPROP; then
		echo "install=1" > $AROMA/check_n9.prop
	fi
	exit 10
fi
...
if [ $OPTION == "config_backup" ]; then
	...
	cd $CONFIG
	[ -f "$CONFIG/check_n9.prop" ] && rm -f $CONFIG/check_n9.prop
	[ -f "$CONFIG/deodexed.prop" ] && rm -f $CONFIG/deodexed.prop
	[ -f "$CONFIG/n960f.prop" ] && rm -f $CONFIG/n960f.prop
...
if [ $OPTION == "kernel_flash" ]; then
	...
	# Flash new Image
	dd if=$TGPTEMP/boot-n9.img of=$BLOCK/BOOT
	sync
	exit 10
fi
